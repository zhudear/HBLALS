...
class SearchBlock(nn.Module):
    ...
    def __init__(self, channel,operator):
...
class DE3(nn.Module):
    ...
    def forward(self,img):
        ...
        for b in range(img.shape[0]):
            ...
            sum=sum+8-res
            #print(b,res)
        return 0.1*sum#0=255 range max is 8
#import VIF_loss
...
class Gray_world_Loss(nn.Module):
    def __init__(self):
        ...
        self.l1 = nn.L1Loss()  # nn.MSELoss()
    def forward(self, target):
        #print(target.shape)#torch.Size([3, 400, 600])
        '''B, G, R = np.double(orgImg[:, 0,:,:]), np.double(orgImg[:, 1,:,:]), np.double(orgImg[:, 2,:,:])
        B_ave, G_ave, R_ave = np.mean(B), np.mean(G), np.mean(R)'''
        ...
        tB, tG, tR = target[:, 0, :, :],target[:,1, :, :], target[:,2, :, :]
        tB_ave, tG_ave, tR_ave = torch.mean(tB), torch.mean(tG), torch.mean(tR)
        #print(tB_ave, tG_ave, tR_ave )
        #loss_Gray=self.l1(tB_ave, tG_ave)+self.l1(tG_ave, tR_ave)+self.l1(tB_ave, tR_ave)
        loss_Gray=torch.abs(tB_ave-tG_ave)+torch.abs(tG_ave- tR_ave)+torch.abs(tB_ave-tR_ave)
        return loss_Gray#100*
#from UISM import UISM
#from UCIQE_tensor import UCIQE
class Network(nn.Module):
    ...
    def __init__(self,lw,operator):
        ...
        #self.LP=LossFunctionlP()#08
        #self.gradient=LossFunctiongradient()
        '''self.loss_fn_alex = lpips.LPIPS(net='alex')
        for k in self.loss_fn_alex.parameters():
            k.requires_grad = False'''
        #self.GW=Gray_world_Loss()
        ...
        self.e=Encoder()
        self.p=DenoiseNetwork(self.nrm_nums,32,self.operator)
        self.d =Decoder()
        #self.sigmoid = nn.Sigmoid()
        ...
        #self.hyper_10 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        #self.hyper_11 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        # 初始化
...
#epoch 60:  0.591911 0.000000 0.000000 0.000702 5.892449 0.000000 3.562198 2.124476 1.323368
    ...
    def _initialize_alphas(self): # sum(1 for i in range(self.illu_layers))
        ...
        for i in range(self.nrm_nums):
            a = torch.tensor(1e-3 * torch.randn(k_denoise, num_ops).cuda(), requires_grad=False)
        ...
        #print('_initialize_alphas',a.shape,self.alphas_denoises[0].shape)#torch.Size([2, 4]) torch.Size([4, 7])
        '''vgg feature layers'''
    ...
    def hyper_parameters(self):
        ...
        return [self.hyper_1, self.hyper_2, self.hyper_3, self.hyper_4,self.hyper_5
            ,self.hyper_6,self.hyper_7, self.hyper_8,#self.hyper_9,self.hyper_10,  #self.hyper_7, self.hyper_8,
                self.vgg1,self.vgg2,self.vgg3,self.vgg4]# ,self.hyper_5]
    ...
    def _losssupernet(self, input, target,vgg_model,loss_fn_alex,discriminator,type):
        #print('_loss')
        ...
        if (type==1):# True:# False:# train
            ...
            vggloss = 0#self.hyper_5 * self.vgg_criterion(vgg1, vgg2, vgg3, vgg4, output, target, vgg_model)
            ...
            #lploss=self.hyper_11*self.LP(output, target, 0.5)
    ...
    def _loss(self, input, target,vgg_model,loss_fn_alex,discriminator,type):
        ...
        if (type==1):# True:# False:# train
            ...
            vggloss = self.hyper_5 * self.vgg_criterion(vgg1, vgg2, vgg3, vgg4, output, target, vgg_model)

            lpipsloss = self.hyper_6 *   loss_fn_alex(output, target)
            tvloss = self.hyper_7 * self.tv(output)
            ...
            #print(out255)
            #vifloss = self.hyper_10 * self.uciqe(output)#0.00001* (12-self.uism(out255))
            #vifloss = self.hyper_10 * self.gw(output)
            #lploss=self.hyper_11*self.LP(output, target, 0.5)
            #deloss = self.hyper_7 * self.discrete_entropy(img)
